...
- @returns {any[]} the array without nulls and undefined values
  ...
  return toArray(arr).filter((item) => item != null && item !== false);
  }

export function filterArrays(arr) {
...
const operations = {
[ARRAY_DIFF_OP.ADD]: (item, index) => {
array.splice(index, 0, item);
},
[ARRAY_DIFF_OP.REMOVE]: (index) => {
array.splice(index, 1);
},
[ARRAY_DIFF_OP.MOVE]: (from, index) => {
array.splice(index, 0, array.splice(from, 1)[0]);
},
};

diffSeq.forEach(({ op, item, index, from }) => {
const operation = operations[op];
if (operation) {
operation(item, index, from);
}
});
