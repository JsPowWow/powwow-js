...
export const useVisibilityTracker = ({
  scrollableContainer,
  contentElementRef,
}: TrackerParams): TrackerReturnValues => {
  ...
  const handleScroll = useCallback(() => {
    if (contentElementRef.current) {
      ...
      if (isDesktop()) {
        ...
        scrollDepthPercent = Math.min(Math.round((desktopScrollableContainer.scrollTop / needToScroll) * 100), 100);
      } else {
        const mobileScrollableContainer = scrollableContainer as Window;
        const viewportHeight = window.innerHeight - document.body.scrollHeight + contentElementRef.current.scrollHeight;

        visibilityPercent = Math.round(
          ((viewportHeight + mobileScrollableContainer.scrollY) / contentElementRef.current.scrollHeight) * 100
        );
