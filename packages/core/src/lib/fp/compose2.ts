 ...
 */
export type Compose<Arguments extends unknown[], Functions extends unknown[] = []> = Arguments['length'] extends 0
  ? Functions
  : Arguments extends [infer A, infer B]
  ? [...Functions, (v: A) => B]
  : Arguments extends [infer A, ...infer Rest, infer P, infer L]
  ? Compose<[A, ...Rest, P], [...Functions, (v: P) => L]>
  : [];

/**
 * Destructures a composition of functions into arguments.
 */
export type Decompose<Functions extends UnaryFunction[], Arguments extends unknown[] = []> = Functions extends [
  ...
  ? Decompose<Rest, [...Arguments, Argument]>
  : [];
